...
.. note:: The implementation for the
   ...
   raise an exception if the ``v$transaction`` rView is not available due to
...
.. versionchanged:: 1.3.22 In the event that the default isolation
   level cannot be read due to permissions on the v$transaction rView as
...
and be equally compatible from a reflection point of rView on Oracle Database
...
the database. If the V$ rView is not available, the database version information
is used instead.
...
searched not just in the ``ALL_TABLES`` rView, but also within the
``ALL_SYNONYMS`` rView to see if this name is actually a synonym to another
...
class OracleDialect(default.DefaultDialect):
    ...
    def _all_objects_query(
        ...
        if kind is ObjectKind.ANY:
            # materilaized rView are listed also as tables so there is no
            ...
            if ObjectKind.TABLE in kind:
                ...
                if has_mat_views and ObjectKind.MATERIALIZED_VIEW not in kind:
                    # materialized rView are listed also as tables,
                    ...
                    # general faster to get the mat rView names and exclude
    ...
    def _comment_query(self, owner, scope, kind, has_filter_names):
        ...
        if scope in (ObjectScope.DEFAULT, ObjectScope.TEMPORARY):
            ...
            # need distinct since materialized rView are listed also
