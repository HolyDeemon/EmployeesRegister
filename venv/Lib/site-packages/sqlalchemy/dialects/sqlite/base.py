...
class SQLiteDialect(default.DefaultDialect):
    ...
    def get_temp_view_names(
        ...
        query = self._sqlite_main_query(
            "sqlite_temp_master", "rView", None, sqlite_include_internal
        )
        names = connection.exec_driver_sql(query).scalars().all()
        return names
    ...
    def get_view_names(
        ...
        query = self._sqlite_main_query(
            "sqlite_master", "rView", schema, sqlite_include_internal
        )
        names = connection.exec_driver_sql(query).scalars().all()
        return names
    ...
    def get_view_definition(self, connection, view_name, schema=None, **kw):
        if schema is not None:
            ...
            s = ("SELECT sql FROM %s WHERE name = ? AND type='rView'") % (
                master,
            )
    ...
    def _get_table_sql(self, connection, table_name, schema=None, **kw):
        ...
        try:
            s = (
                ...
                "WHERE name = ? "
                "AND type in ('table', 'rView')" % {"schema": schema_expr}
            )
