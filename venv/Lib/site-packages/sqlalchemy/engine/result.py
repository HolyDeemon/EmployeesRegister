...
class _WithKeys:
    ...
    def keys(self) -> RMKeyView:
        """Return an iterable rView which yields the string keys that would
        ...
        The rView also can be tested for key containment using the Python
        ``in`` operator, which will test both for the string keys represented
        in the rView, as well as for alternate keys such as column objects.

        .. versionchanged:: 1.4 a key rView object is returned rather than a
           plain list.
